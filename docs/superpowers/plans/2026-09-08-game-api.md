...
- **모든 엔드포인트는 응답 DTO 를 가진다.** 컨트롤러 메서드에 `Promise<XxxResponseDto>` 리턴 타입을 명시한다. `any` 나 리턴 타입 누락 금지 — swagger 응답 스키마가 비면 FE 가 응답 모양을 모른다.
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: 'QuestionRepository', useValue: questions },
      ],
    })
